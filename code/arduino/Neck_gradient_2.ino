...
int status_over = 0;                // 정상일경우 0, 17도 보다 클경우에는 1, 20보다 클 경우 2;   
...
void loop() { // run over and over
          ...
          if(( Now_Millis - Start_Millis_10minute )>= 600000)  // 10분이 지났는지 확인
               {
          ...
          if(( Now_Millis - Start_Millis_Temp )>= 1000) // 0.5초 
            {
//               getAngle_MPU6050();// X축 기울기 각도를 읽어서 전역변수(angle_x)에 넣는다.
               ...
               if(angle_x >= 17)  //17도 보다 클경우. 
                 {
                   ...
                   else if ((status_over >= 1) &&  (Now_Millis-Start_Millis_17 >= 6000)) 
                      {
...
//                Serial.print("count_Num = "); Serial.println(count_Num);
                ...
                Serial.print("sum_angle_x = "); Serial.println(sum_angle_x);
//                Serial.print("average_x = "); Serial.println(average_x);
...
void getAngle_MPU6050() 
 {
  ...
  angle_x = int(mpu.getAngleX());
  angle_y = int(mpu.getAngleY());
  angle_z = int(mpu.getAngleZ());
  }

// 숫자의 자리수를 1,000cm로 맞춰서 보냄. 500cm가 최고 임.)
void BT_send(int err_code ) // error code - 0: 정상,  1: 1분이상 17~20도, 2:1분이상 20도 이상
{

 BT_Ble.print(String(angle_x)+","+String(average_x)+","+String(err_code)); //블루투스 모듈로 송신
}
